...
for i in xrange(len(app_rej2.list_d)):
    app_rej2['Year'][i]=app_rej2.list_d[i].year
    app_rej2['Month'][i]=app_rej2.list_d[i].month
...
numerical_features = app_rej2.get(['loan_amnt', 'dti','emp_length','addr_state_1','Year','Month'])
...
##Generating an ROC curve Test for Naive Bayes
from sklearn.metrics import roc_curve
y_pred = gnb.predict_proba(features_test)
y_score = np.max(y_pred, axis=1)
fpr, tpr, th = roc_curve(target_test, y_score)
plt.plot([0, 1], [0, 1], 'k--')
...
fpr, tpr, thresholds = roc_curve(target_train, target_predicted_train,pos_label=4) # recall my labels are 4,5 and 6
roc_auc = auc(fpr, tpr)
print("Area under the ROC curve : %f" % roc_auc)
...
cm = confusion_matrix(target_train, target_predicted_train)
plt.matshow(cm)


##Generating an ROC curve TRAIN
from sklearn.metrics import roc_curve
y_pred = lr.predict_proba(features_train)
y_score = np.max(y_pred, axis=1)
fpr, tpr, th = roc_curve(target_train, y_score)
roc_auc = auc( tpr , fpr)
print "Area under the ROC curve : %f" % roc_auc
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('False Positive Rate')
...
##Generating an ROC curve Test
from sklearn.metrics import roc_curve
y_pred = lr.predict_proba(features_test)
y_score = np.max(y_pred, axis=1)
fpr, tpr, th = roc_curve(target_test, y_score)
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('False Positive Rate')
